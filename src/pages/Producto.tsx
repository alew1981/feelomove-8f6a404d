...
const Producto = () => {
  ...
  const { data: artistOtherCities } = useQuery({
    ...
    queryFn: async () => {
      ...
      (festivalsRes.data || []).forEach((event: any) => {
        ...
        if (attractionNames.some((name: string) => name.toLowerCase().includes(artistForSearch.toLowerCase())) && event.venue_city) {
          const existing = cityMap.get(event.venue_city);
          if (existing) {
            existing.count++;
          } else {
  ...
  const stay22Urls = useMemo(() => {
    ...
    const eventDateStr = isFestivalRoute ? ((eventDetails as any)?.start_date || eventDetails?.event_date) : eventDetails?.event_date;
  ...
  const ticketPrices = useMemo(() => {
    ...
    try {
      const ticketData: TicketTypesData = typeof rawTicketTypes === 'string' ? JSON.parse(rawTicketTypes) : rawTicketTypes;
      ...
      for (const priceType of ticketData.price_types) {
        if (priceType.price_levels && Array.isArray(priceType.price_levels)) {
          for (let levelIndex = 0; levelIndex < priceType.price_levels.length; levelIndex++) {
            ...
            const normalizedAvailability = [\"none\", \"soldout\", \"sold_out\"].includes(rawAvailability) ? \"none\" : rawAvailability === \"limited\" ? \"limited\" : \"available\";
  ...
  const isArtistEntry = isFestivalDisplay && (eventDetails as any).secondary_attraction_name && (eventDetails as any).secondary_attraction_name !== (eventDetails as any).primary_attraction_name;
  const displayTitle = isArtistEntry ? (eventDetails as any).secondary_attraction_name : eventDetails.event_name;
  const displaySubtitle = isArtistEntry ? `en ${(eventDetails as any).primary_attraction_name || eventDetails.event_name}` : null;
  ...
  const hasVipTickets = ticketPrices.some(t => /vip/i.test(t.type || '') || /vip/i.test(t.description || '')) || (eventDetails as any).has_vip_tickets;
          ...
          <section className=\"relative w-full bg-muted overflow-hidden\">
            ...
            <div className=\"container relative z-10 mx-auto px-4 py-8 lg:py-16\">
              ...
              <div className=\"grid grid-cols-1 lg:grid-cols-12 gap-8 items-center\">
                <div className=\"lg:col-span-12\"> {/* Se ajusta a 12 columnas para centrar si se desea, o mantener el flujo */}
                  <div className=\"space-y-6\">
                    <div className=\"space-y-2\">
                      <div className=\"flex flex-wrap gap-2 mb-4\">
                        <Badge variant=\"secondary\" className=\"bg-accent/10 text-accent-foreground border-accent/20 backdrop-blur-md\">
                          {eventDetails.primary_category_name || 'Evento'}
                        </Badge>
                        {hasVipTickets && (
                          <Badge variant=\"outline\" className=\"bg-yellow-500/10 text-yellow-500 border-yellow-500/20 backdrop-blur-md\">
                            VIP Disponible
                          </Badge>
                        )}
                      </div>
                    ...
                    <div className=\"flex flex-col sm:flex-row sm:items-center gap-4 text-muted-foreground\">
                      <div className=\"flex items-center gap-2 bg-secondary/50 px-3 py-1.5 rounded-full backdrop-blur-sm\">
                        ...
                        <span className=\"font-medium\">{eventDetails.venue_name}, {eventDetails.venue_city}</span>
                      </div>
                    </div>
                  </div>
                </div>
                
                {/* LA IMAGEN MINIATURA QUE ESTABA AQUÍ HA SIDO ELIMINADA PARA MEJORAR EL PERFORMANCE */}
              </div>
            </div>
          </section>
                ...
                <section id=\"tickets\" className=\"scroll-mt-24\">
                  <div className=\"flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6\">
                    <div className=\"flex items-center gap-3\">
                      ...
                      <Badge variant=\"outline\" className=\"rounded-full\">{ticketPrices.length}</Badge>
                    </div>
                  ...
                  {ticketPrices.length === 0 ? (
                    <Card className=\"bg-muted/30 border-dashed\">
                      <CardContent className=\"flex flex-col items-center justify-center py-12 text-center\">
                        ...
                        <p className=\"text-sm text-muted-foreground\">Vuelve a consultar más tarde para nuevas actualizaciones.</p>
                      </CardContent>
                    </Card>
                  ) : (
                    <div className=\"space-y-4\">
                          ...
                          <CardContent className=\"p-0\">
                            <div className=\"flex flex-col sm:flex-row sm:items-center p-5 gap-4\">
                              <div className=\"flex-1 space-y-1\">
                                ...
                                <p className=\"text-sm text-muted-foreground\">{ticket.description || 'Entrada para el evento'}</p>
                                {ticket.availability === \"limited\" && (
                                  <Badge variant=\"outline\" className=\"text-orange-500 border-orange-500/20 bg-orange-500/5 text-[10px] uppercase tracking-wider\">Últimas unidades</Badge>
                                )}
                              </div>
                              ...
                              <div className=\"flex items-center justify-between sm:justify-end gap-6\">
                                <div className=\"text-right\">
                                  ...
                                  <span className=\"text-[10px] text-muted-foreground uppercase tracking-tighter\">+ {ticket.fees}€ gastos gestión</span>
                                </div>
                      ...
                      {ticketPrices.length > 4 && (
                        ...
                        >
                          {showAllTickets ? \"Ver menos opciones\" : `Ver las ${ticketPrices.length} opciones de entradas`}
                        </Button>
                      )}
                    </div>
                  )}
                  ...
                  <CardContent className=\"p-6 space-y-6\">
                    {!cart || cart.tickets.length === 0 ? (
                      <div className=\"text-center py-8 space-y-3\">
                        ...
                        <p className=\"text-sm text-muted-foreground font-medium\">Selecciona tus entradas para empezar a configurar tu viaje</p>
                      </div>
                    ) : (
                      <div className=\"space-y-6\">
                              ...
                              <div className=\"flex justify-between items-center text-sm\">
                                <div className=\"flex items-center gap-2\">
                                  ...
                                  <div>
                                    ...
                                    <p className=\"text-[10px] text-muted-foreground uppercase font-bold mt-1\">Hotel en {eventDetails.venue_city}</p>
                                  </div>
                                </div>
