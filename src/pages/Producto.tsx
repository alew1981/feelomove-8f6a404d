...
const Producto = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("vw_events_with_hotels").select("*").eq("event_id", id).maybeSingle();
  ...
  const formattedDate = format(eventDate, "EEEE, d 'de' MMMM 'de' yyyy", {
    locale: es
  });
  ...
  // Parse hotels
  const hotels = Array.isArray(eventDetails.hotels) ? eventDetails.hotels : [];
  return <div className="min-h-screen bg-background">
      <Navbar />
      ...
      <main className="container mx-auto px-4 py-8 mt-20">
        ...
        <div className="relative rounded-lg overflow-hidden mb-8 h-[500px]">
          <img src={eventDetails.image_large_url || eventDetails.image_standard_url || "/placeholder.svg"} alt={eventDetails.event_name} className="w-full h-full object-cover" />
          ...
          {/* Favorite Button */}
          <Button variant="ghost" size="icon" className="absolute top-6 left-6 bg-white/10 backdrop-blur hover:bg-white/20" onClick={() => toggleFavorite({
          event_id: id!,
          ...
          <div className="absolute bottom-6 left-6 right-6 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-white/90 font-medium">
              ...
              {eventDetails.venue_name && <div className="flex items-center gap-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            {/* Artists */}
            {eventDetails.attraction_names && eventDetails.attraction_names.length > 0 && <Card className="border-2 border-border">
                <CardHeader className="bg-[#121212] text-white">
                  <CardTitle className="flex items-center gap-2 uppercase tracking-wide text-sm">DESCRIPCIÓN<Music className="h-5 w-5 text-[#00FF8F]" />
                    Artistas
                  </CardTitle>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="flex flex-wrap gap-2">
                    {eventDetails.attraction_names.map((artist: string, index: number) => <Badge key={index} className="bg-[#121212] text-white text-sm px-4 py-2 rounded-md">
                        {artist}
                      </Badge>)}
                  </div>
                </CardContent>
            ...
            {ticketPrices.length > 0 && <Card className="border-2 border-border overflow-hidden">
                <CardHeader className="bg-[#121212] text-white">
                ...
                <CardContent className="p-0">
                  {ticketPrices.map((ticket: any, index: number) => <div key={index} className="p-6 border-b border-border last:border-0 hover:bg-[#F2F2F2] dark:hover:bg-[#1a1a1a] transition-colors">
            ...
            {hotels.length > 0 && <div>
                ...
                <div className="space-y-4">
                  {hotels.slice(0, 5).map((hotel: any) => <Card key={hotel.hotel_id} className="border-2 border-border overflow-hidden hover:shadow-card-hover transition-all">
                      <div className="flex flex-col md:flex-row">
                        <div className="md:w-1/3 h-48 md:h-auto relative">
                          <img src={hotel.hotel_main_photo || hotel.hotel_thumbnail || "/placeholder.svg"} alt={hotel.hotel_name} className="w-full h-full object-cover" />
                        ...
                        <div className="md:w-2/3 p-6">
                          <div className="flex items-start justify-between mb-3">
                            <div>
                              ...
                              {hotel.hotel_chain && <p className="text-sm text-muted-foreground">{hotel.hotel_chain}</p>}
                            </div>
                            {hotel.hotel_review_score && <div className="text-right">
            ...
            <Card className="sticky top-24 border-2 border-border">
              ...
              <CardContent className="pt-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="font-medium">{format(eventDate, "d MMM yyyy", {
                    locale: es
                  })}</p>
                ...
                {eventDetails.has_hotel_offers && eventDetails.package_price_min && <div className="p-4 bg-[#00FF8F]/10 rounded-md border border-[#00FF8F]/20">
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    {eventDetails.estimated_package_savings && eventDetails.estimated_package_savings > 0 && <p className="text-xs text-[#00FF8F] font-bold mt-1">
                        Ahorra €{Number(eventDetails.estimated_package_savings).toFixed(0)}
                      </p>}
                  </div>}

                <div className="pt-4 space-y-3">
                  {eventDetails.event_url && <Button variant="primary" className="w-full h-12 text-base" asChild>
                      <a href={eventDetails.event_url} target="_blank" rel="noopener noreferrer">
                        Reservar Entradas
                      </a>
                    </Button>}
                  
                  {eventDetails.has_hotel_offers && <Button variant="outline" className="w-full h-12">
                      Ver Paquetes
                    </Button>}
                </div>
